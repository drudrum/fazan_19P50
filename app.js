...
function processResponse(){
  ...
  if (response.length<=2){
    operationData.cb.apply(operationData,['Очень короткий ответ']);
  ...
  if (!rcvCrc.equals(bufCrc)){
    ...
    operationData.cb.apply(operationData,['Неверный CRC']);
    responseCbTimeout=null;
    trySend();
    return;
  }
  ...
  if ((Buffer.from('80','hex')).readUInt8(0)+operationData.cmd.readUInt8(1)==buf.readUInt8(1)) {
    ...
    operationData.cb.apply(operationData,[err+' ErrCode:'+errCode+' response:'+buf2hex(buf)]);
    responseCbTimeout=null;
    trySend();
    return;
  }

  //global.debugLevel>2 && console.log('Response:' + buf2hex(buf));
  operationData.cb.apply(operationData,[null,buf]);
  responseCbTimeout=null;
  trySend();
}
